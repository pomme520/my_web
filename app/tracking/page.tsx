...
export default function Tracking() {
  ...
  const [result, setResult] = useState<{ orderNumber: string; status: string; customerName: string; description: string } | null>(null);
    ...
    <div className="mx-auto max-w-5xl px-6 py-16 lg:px-12">
      ...
      <div className="mt-10 border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row">
          <input value={number} onChange={(e) => setNumber(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && search()} placeholder="例如 R-123456" className="min-w-0 flex-1 rounded-md border border-slate-300 p-3 outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" />
      ...
      {result && <div className="mt-6 border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm text-slate-500">案件編號</p><p className="text-2xl font-black text-slate-900">{result.orderNumber}</p></div>
          <span className="w-fit rounded-full bg-blue-100 px-4 py-2 font-bold text-blue-800">{result.status}</span>
        </div>
        <div className="grid gap-6 p-6 md:grid-cols-2">
          <div><p className="text-sm text-slate-500">申請人</p><p className="mt-1 font-bold">{result.customerName}</p></div>
          <div className="md:col-span-2"><p className="text-sm text-slate-500">問題描述</p><p className="mt-1 leading-7 text-slate-700">{result.description}</p></div>
        </div>
      </div>}
    </div>
  );
