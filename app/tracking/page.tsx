...
export async function POST(request: Request) {
  ...
  if (!body.customerName || !body.phone || !body.department || !body.deviceType || !body.issueType || !body.description) {
    return NextResponse.json({ message: '請完整填寫必填欄位' }, { status: 400 });
  }

  let orderNumber = generateOrderNumber();
  while (orders.some((order) => order.orderNumber === orderNumber)) orderNumber = generateOrderNumber();
