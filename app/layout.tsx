            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-5 lg:px-12">
              <Link href="/" className="flex items-center gap-5" aria-label="台灣電力公司台東區營業處首頁">
                ...
                <div className="flex flex-col justify-center leading-[0.9] text-[#111111]">
                  <span className="brand-chinese text-[clamp(2rem,2.6vw,3.4rem)] font-black tracking-[-0.05em]">
                    台灣電力公司
                  </span>
                  <span className="mt-1 text-[clamp(1.05rem,1.5vw,2rem)] font-semibold tracking-[0.08em] text-[#111111]">
              ...
              <nav aria-label="主要導覽" className="space-y-1">
                {navigation.map(([href, label]) => (
                  <Link
                    key={href}
                    href={href}
                    className="block rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-blue-50 hover:text-blue-700"
                  >
                    {label}
                  </Link>
                ))}
