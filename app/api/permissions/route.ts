...
import { PermissionName, Role, permissionNames, roles } from '@/lib/permissions';
...
export async function PUT(request: Request) {
  ...
  await prisma.$transaction(roles.flatMap((role) => permissionNames.map((name) => prisma.permission.upsert({ where: { role_name: { role, name } }, update: { enabled: body.settings[role]?.[name] === true }, create: { role, name, enabled: body.settings[role]?.[name] === true } }))));
  return GET(request);
}
